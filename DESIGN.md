...
| StrengthMeter | Four 4 px segments under a password field, filled to the reading in `status.danger` / `status.warning` / `status.success`, with the reading named in words beneath. The rule it draws is `estimatePasswordStrength` in `@helpdock/schemas`, which the api enforces, so the meter never promises what the server will refuse. |
...
| 2026-09-19 | 1.3. Added StepProgress and StrengthMeter to §6.2 for the first-run wizard (`Admin/Wizard`). A step that is merely later uses `text.secondary`, not `text.disabled`: the latter is 2.3:1 on the canvas, which §10 does not allow for a label anyone is meant to read. |
