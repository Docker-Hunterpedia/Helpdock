...
export class DepartmentsService {
  ...
  async addMember(
    ...
    if (
      person === undefined ||
      !isEligibleMember(
        { role: person.role, departmentIds: person.departmentIds ?? 'all' },
        departmentId,
      )
    ) {
      // Deliberately one answer for "no role in this brand", "deactivated" and
      // "works in other departments": all three mean the same thing to the
      // person adding them, and telling them apart would say who works where.
