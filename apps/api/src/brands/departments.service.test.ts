...
/**
 * Audit rows are collected; the one read that reaches past the repository —
 * `ticketsInDepartment`, which counts over `tickets` — answers none. The count
 * itself is proved against Postgres in `brands.integration.test.ts`, and the
 * case where it is not zero is `department-deletion.test.ts`.
 */
const tx = {
  insert: () => ({
...
describe('membership', () => {
  ...
  it('never offers a team leader somebody they may not manage (DOMAIN-RULES §1.2)', async () => {
    const { members } = await service.eligibleMembers(
      contextFor(actor('team_leader', [BILLING])),
      BILLING,
    );

    // The admin reaches Billing but is above a team leader's ceiling; the
    // outsider is below it but cannot see Billing.
    expect(members.map((member) => member.userId)).toEqual([AGENT]);
  });

  it('refuses a team leader adding an admin to their own team', async () => {
    const failure = await refusalOf(
      service.addMember(
        contextFor(actor('team_leader', [BILLING])),
        { departmentId: BILLING, teamId: FRONT_LINE },
  ...
  it('refuses a team leader taking an admin off a team, the same ceiling backwards', async () => {
    repository.memberships = [{ teamId: FRONT_LINE, userId: ADMIN }];
  ...
  it('lets an admin add the admin a team leader could not', async () => {
    ...
    expect(teams[0]?.members.map((member) => member.userId)).toEqual([ADMIN]);
  });

  it('lets anyone who leads the department remove somebody whose role is gone', async () => {
    repository.memberships = [{ teamId: FRONT_LINE, userId: 'left-the-brand' }];
