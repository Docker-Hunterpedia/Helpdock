...
| `onClosedForCsat` | The same, **unless** the ticket is merged or the status is `excluded_from_reports` | M1-12: writes `csat.requested` to the outbox when the brand has CSAT on |
...
status's `excluded_from_reports` and the ticket's `merged_into_id`, never off a
name) runs `onClosedForCsat`, which — when the brand has CSAT on — writes
