...
Helpdock is pre-alpha. The stack described here comes up and serves the admin
sign-in screen; the first-run wizard, real sign-in and everything after it are
still being built ([PRD §4](../planning/PRD.md)).
...
The first-run wizard — admin account, first brand, SMTP test — is deliverable
M0-08 and is not built yet. Until it lands you will see the admin **sign-in
screen**, which runs against an in-memory fixture rather than the database; the
credentials for it are in
[`apps/admin/README.md`](../../apps/admin/README.md). Nothing you do there is
stored. Real sign-in arrives with M0-05.
...
- **`.env`.** It holds `APP_MASTER_KEY`, and without that key every stored
  secret is lost. Keep it somewhere other than the server.
