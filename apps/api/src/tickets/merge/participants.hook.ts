 ...
 * and M1-13's `ParticipantsMergeHook` (`participants/merge-participants.hook.ts`)
 * replaces this provider in `TicketsModule` with one that does it —
...
export interface MergeParticipantsEvent {
  ...
  /** Who merged or unmerged, for the participant's `added_by` and the activity row. */
