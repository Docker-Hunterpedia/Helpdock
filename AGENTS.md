...
Current state: pre-alpha. **M0 Skeleton shipped on 2026-09-19** — see [docs/completed/M0-skeleton.md](docs/completed/M0-skeleton.md) for what was built and the ten gaps it left open. **M1 Ticketing core shipped on 2026-09-25** — see [docs/completed/M1-ticketing-core.md](docs/completed/M1-ticketing-core.md) for what was built, the exit criterion met in part (tagging a ticket in the workspace) and the gaps it left open. M2 Email channel, M3 Automation and SLAs and M5 Help center depend only on M1 and may start; none has started yet (see the status board in the PRD).
