...
Screenshot baselines live under `apps/admin/e2e/__screenshots__/<locale>/` and
are **Linux pixels**, because the comparison runs on `ubuntu-latest`. They are
generated by CI, not on a laptop:
[`.github/workflows/screenshots.yml`](../../.github/workflows/screenshots.yml)
renders them on the same runner image, with the same
`playwright install --with-deps chromium`, as the step that compares them, and
uploads them as an artifact called `screenshot-baselines`. Run it from the
Actions tab or with `gh`, then commit what comes back:
...
git add apps/admin/e2e/__screenshots__
```

It also runs by itself on a pull request that touches `apps/admin/src/**`,
`packages/ui/src/**` or the spec, so a deliberate layout change arrives with its
new baselines attached to the run.

The spec stays tagged `@screenshot` and stays out of `pnpm e2e`. That is not a
leftover: a comparison against Linux pixels fails on macOS and on Windows, and
`pnpm e2e` has to be a command a contributor can run. CI runs the comparison as
its own step, `pnpm --filter @helpdock/admin e2e:screenshots`, which is also the
command to run on a Linux machine. `e2e:baselines` regenerates them locally
through `mcr.microsoft.com/playwright`; the admin README explains why the dev
...
### Routes name their input schema

Every `@Param`, `@Query` and `@Body` on a controller handler passes its schema
explicitly:

```ts
@Post('sign-in')
@Public()
async signIn(@Body(new ZodValidationPipe(SignInRequestDto)) body: SignInRequestDto) {}
```

Not because the global pipe does not work, but because it works *conditionally*.
nestjs-zod's global `ZodValidationPipe` finds a DTO's schema through the
`design:paramtypes` metadata the compiler emits for a decorated parameter. An
`import type` erases that metadata — and Biome's `useImportType` rewrites any DTO
used only as a type into one — so validation stops without a word. A transform
that never emits the metadata at all, like the esbuild Vitest uses, removes it
everywhere at once. That is what
[issue #36](https://github.com/Docker-Hunterpedia/Helpdock/issues/36) was:
`GET /api/brands/:brandId` had not been validating anything, and its 400 for a
malformed id came from the permission guard.

`pnpm check:validation` fails the build for a parameter that names no schema. It
shares its scanner with `pnpm check:routes`
([`scripts/controller-scan.ts`](../../scripts/controller-scan.ts)).

Two things the check leaves alone, on purpose:

- **Socket events.** `@MessageBody()` on the `/staff` gateway is typed `unknown`
  and parsed inside the handler with `parseMessage(schema, body)`, because a
  pipe cannot answer through an acknowledgement.
- **Which values a schema allows.** `oauthProviderParamSchema` bounds the
  `:provider` segment but does not list the providers, because both OAuth routes
  answer a browser with a redirect and a pipe can only answer with a body. The
  handler maps an unknown provider to a refusal the sign-in screen renders.

Guards run before pipes, so a brand-scoped route's `:brandId` is read by the
permission guard first. It refuses a malformed one with `brandIdParamSchema` —
the same schema the handler names — so the response carries
`fields: [{ path: "brandId" }]` either way.
...
80 % on `packages/*` is the number in ARCHITECTURE §15. The two apps have gates
of their own for opposite reasons. `apps/api` has no UI, so nothing else covers
it — and it is measured across both suites because half of it is only reached
over HTTP, and a figure from the unit run alone would say more about where the
tests are than about what is covered. `apps/admin` is measured by Vitest for its
logic and by Playwright for its layout, so its gate is lower; it was raised from
70 with M0-06 and held there by M0-08, because those screens decide what a
person may do and whether an install can be finished at all.

Raising a gate is a pull request of its own. Lowering one needs a reason in the
...
They do cover input validation, which they did not until M0-11: every route now
names its schema on the parameter rather than leaving it to the global pipe's
`design:paramtypes` metadata, which esbuild — what Vitest transforms with — does
not emit. [Routes name their input schema](#routes-name-their-input-schema) is
the rule and `pnpm check:validation` is what keeps it.
...
Five workflows, under [`.github/workflows/`](../../.github/workflows/). Every
third-party action in all of them is pinned to a full commit sha with the
version in a comment beside it, because a moving tag is a moving dependency
nobody reviews; Renovate proposes the bumps.
...
Testcontainers starts its own containers and no service container is declared.
...
[`renovate.json`](../../renovate.json): pull requests on Monday mornings,
security advisories at any time and at a higher priority. GitHub Actions bumps
are one pull request, minor and patch dev-dependency bumps another, and the
`@opentelemetry/*` and `@nestjs/*` families are grouped because a mixed set of
either does not work. Node stays below 25 until the stack table in
ARCHITECTURE §1 says otherwise.

### Releases

[`docs/guides/release.md`](release.md) is the procedure: a changeset per change,
a "version packages" pull request that Changesets keeps open on `main`, and a
`v*` tag that publishes the multi-architecture image to GHCR with a CycloneDX
