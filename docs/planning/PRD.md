...
- [x] `docker compose up` on a clean machine reaches the wizard and creates an admin and a brand.
- [ ] All four auth methods and TOTP work end-to-end in a Playwright test. **Partly**: password and TOTP are; sign-in links are covered at the HTTP layer instead, and Google and GitHub wait on the OAuth credentials listed under external dependencies. See [the milestone doc](../completed/M0-skeleton.md#exit-criteria).
- [x] Every negative test in DOMAIN-RULES §1.6 passes and is required in CI.
- [x] Killing the relay between commit and publish loses no job; a rolled-back transaction enqueues nothing.
- [x] Two admin sessions on different api replicas see each other's presence change within 5 seconds.
- [x] CI is green and is a required check on `main`.
- [x] [`docs/completed/M0-skeleton.md`](../completed/M0-skeleton.md) describes what was built.
...
- [ ] An agent can create, assign, reply to, note, tag, merge, split and close tickets in the admin UI, in English and Arabic. **Partly**: seven of the eight verbs have browser tests in both locales; the workspace has no control that tags a ticket, because no artboard draws one. See [the milestone doc](../completed/M1-ticketing-core.md#exit-criteria).
...
| 2026-09-25 | M1 shipped. All fifteen deliverables merged (#63, #64, #66, #69, #70, #71, #72, #73, #74); four of five exit criteria met and the first ([create, assign, reply, note, tag, merge, split and close in en and ar](#m1-ticketing-core)) met for seven of eight verbs, because the workspace has no control that tags a ticket and no artboard draws one. Actual effort seven days against an estimate of 6–8 weeks. What was built, and the accepted gaps, are in [docs/completed/M1-ticketing-core.md](../completed/M1-ticketing-core.md#gaps-accepted). M2 Email channel, M3 Automation and SLAs and M5 Help center depend only on M1 and may start. |
